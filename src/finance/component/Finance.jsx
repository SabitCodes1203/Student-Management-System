...
import { DollarSign, CreditCard, Calendar, TrendingUp, TrendingDown, AlertCircle, CheckCircle2, Clock, Receipt, Download } from 'lucide-react';
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <motion.div
        ...
        className="bg-white rounded-2xl p-4 sm:p-6 shadow-md border-2 border-gray-200"
      >
        <h2 className="text-xl sm:text-2xl font-bold text-gray-800 mb-6">Payment Methods</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[
            { name: 'Credit Card', icon: CreditCard, last4: '**** 4532', type: 'Visa', default: true },
            { name: 'Bank Transfer', icon: Receipt, last4: '**** 7890', type: 'Checking', default: false },
            { name: 'Debit Card', icon: CreditCard, last4: '**** 1234', type: 'Mastercard', default: false }
          ].map((method, index) => {
            ...
            return (
              <motion.div
                ...
                className={`p-4 rounded-xl border-2 ${
                  method.default ? 'border-blue-500 bg-blue-50' : 'border-gray-200 bg-white'
                } cursor-pointer transition-all`}
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-lg ${
                      method.default ? 'bg-blue-100' : 'bg-gray-100'
                    }`}>
                      <Icon className={`w-5 h-5 ${
                        method.default ? 'text-blue-600' : 'text-gray-600'
                      }`} />
                    </div>
                    <div>
                ...
                <button className="mt-3 w-full py-2 px-4 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg text-sm font-medium transition-all">
                  {method.default ? 'Manage' : 'Set as Default'}
                </button>
              </motion.div>
            );
          })}
